...
さて、このような直線の上に、モジュールをネスト構造を保ちながら配置することができる必要がある。ちょうど区間のようなものである。たとえば、モジュール `foo` とモジュール `foo.bar` があるとして、モジュール `foo.bar` の区間はモジュール `foo` の区間に収まる必要がある。これを簡略化すれば、 `foo.bar` の定義に依存する `foo` の定義に依存する `foo.bar` の定義が存在していけない、つまり `foo.bar -> foo -> foo.bar` という依存関係があってはいけないということである。
