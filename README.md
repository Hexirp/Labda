...
ラカーセアーを CPS 変換したものから逆算して型を考えることも考察してみたが、型がない変数のそれぞれで継続が必要かどうかを判定することが出来ない。

結局、排中律と選択公理の両方をエミュレートできるだけの埋め込みを構成することは不可能であると言い切るしかなさそうである。排中律と選択公理を Reader モナドで環境に置くことが最適な方法になりそうである。
