...
### Labda は Homotopy Type Theory と同じ同値性の型を持つ。

Labda は高度な数学を記述できる。 Homotopy Type Theory における関数外延性と単一性が成立する同値性の型は、高度な数学を記述するのに役に立つ。

このため、 Labda は Homotopy Type Theory と同じ同値性の型を持つ。
...
### Labda はコンパイル時にリテラルの正しさを検証することができる。

たとえば、 `A` という型があって、その型の値を生成する方法が `new : Integer -> Option A` しかない場合、 `new 3` のように生成するとしても、それが `None` である可能性を考えなければならない。 `new 3` が `Some` になることがわかっている場合、これは無駄である。そこで、多段階計算を使用してコンパイル時に `new 3` を実行して、それが `None` になればコンパイルを失敗させ、 `Some` になれば `A` の値を取り出して埋め込むことができる。

このため、 Labda はコンパイル時にリテラルの正しさを検証することができる。

### Labda は巨大なデータベースも値として扱うことができる。

プログラミングにおいては、文字コードなどのような巨大なデータベースも参照して正しさを検証する必要があることもある。そこで、巨大なデータベースをコンパイル時に参照しながら検証していく必要がある。

このため、 Labda は巨大なデータベースも値として扱うことができる。

### Labda はモジュールに類似する機能を持つ。

名前空間を複数へ分割する機能は、高度なプログラムを記述するために必須である。

このため、 Labda はモジュールに類似する機能を持つ。

### Labda のモジュールは環境を多段階にしたものである。

そもそもモジュールを導入したいのはなぜだろうか？ それは、一つの名前空間だと名前が重複し、コードが巨大になるほど長い名前を使わざるを得なくなるからである。モジュールは名前空間を区切り、それぞれの名前が干渉し合わないようにすることができる。ここでの名前空間とは、すなわち型理論における環境のことにほかならない。

このため、 Labda のモジュールは環境を多段階にしたものである。

### Labda のモジュールはネスト構造を保ちながら定義が線形に並ぶことを条件とする。

Labda のモジュールは環境を多段階にしたものである。これは、どういうことか。たとえば、 `{ x : A; y : B; z : C; ...}` という環境があったとしよう。このとき、 `y` が依存することができるのは、 `x` だけである。こういう意味で、環境は線形である。これをモジュールと捉えてネスト構造を導入すると、 `module root where { x : A; module foo where { y : B }; z : C; ... }` ということになる。

さて、このような直線の上に、モジュールをネスト構造を保ちながら配置することができる必要がある。ちょうど区間のようなものである。たとえば、モジュール `foo` とモジュール `foo.bar` があるとして、モジュール `foo.bar` の区間はモジュール `foo` の区間に収まる必要がある。これを簡略化すれば、 `foo.bar` の定義に依存する `foo` の定義に依存する `foo.bar` の定義が存在していけない、つまり `foo -> foo.bar -> foo` という依存関係があってもよいが `foo.bar -> foo -> foo.bar` という依存関係があってはいけないということである。

人間は一つの線になったものを理解しやすく、そうでないものは理解しにくいのだと私は思う。このため、循環参照は可読性を下げる。これは、それぞれの定義が循環参照している場合はもちろん、「モジュール `A` の `x` がモジュール `B` の `y` に依存していて、モジュール `A` の `z` に依存している」というような、モジュール単位で循環参照している場合も含む。 fumieval さんが 2022 年 5 月 9 日に投稿した文章（[リンク](https://fumieval.hatenablog.com/entry/2022/05/09/113715)）でも、その問題を指摘している。線形に並ぶことを条件とすることで、循環参照を除くことができる。

しかしながら、モジュールについては、もう一つ、「 `foo.bar` が `foo` に依存するべきなのか、あるいは `foo` が `foo.bar` に依存するべきなのか」という問題もある。一つ前の問題が「横」の問題であるとしたら、この問題は「縦」の問題である。そして、この問題は一つ前の問題よりもずっと困難である。なぜならば、基盤ライブラリは、徐々に定義を追加していく末広がりの木構造を持ち、 `foo.bar` が `foo` に依存するという構造と親和性が高いのに対して、アプリケーションは、全てのコードが `main` 関数へ収束していく木構造を持ち、 `foo` が `foo.bar` に依存するという構造と親和性が高いためである。このため、どちらかへと定めると、もう片方のコードを記述するのが困難になるという関係にあるのである。しかし、ネスト構造を保ちながら配置することを条件とすることで、どちらも両立することができる。

このため、 Labda のモジュールはネスト構造を保ちながら定義が線形に並ぶことを条件とする。

### Labda はネストした式を避けてリニアに書くことを助ける機能を持つ。

自然言語は、生成文法に従う入れ子の構造を基本としているというわけではなく、むしろ小さな部品を平坦に連結して並べていることがほとんどであるように、私は思う。そして、人間が読みやすいプログラムのコードというのも、このような自然言語と同様に、できるだけ入れ子の構造を避けて小さな部品を平坦に連結して並べるという特徴を持っているように、私は思う。このように考えて、小さな部品を平坦に連結して並べることが人間が親しみやすい言語の本質であるのではないかと、私は思う。

Labda も、ネストした式を避けてリニアに書くことを助ける機能が必要である。それは、適用に対するメソッドチェーンだったり、ラムダ抽象と適用に対する let 式だったり、リストや配列などを要素をリニアに並べるだけで表せる機能だったり、作用子を使用した式に対する do 式だったり、である。

このため、 Labda はネストした式を避けてリニアに書くことを助ける機能を持つ。

### Labda はインポート宣言の機能を持つ。

Labda はモジュールを持つ。しかし、時には、どのモジュールの定義なのかを省略したい時がある。インポート宣言は、これを省略できるようにする。高度なプログラムを書くためには必須である。

このため、 Labda はインポート宣言の機能を持つ。

### Labda はエクスポート宣言の機能を持つ。

Labda はモジュールを持つ。ここで、モジュールが大量になると、とりあえず同じ機能に関連した複数のモジュールをインポートしておきたい時がある。しかし、これだとインポート宣言が大量になり重複も多くなる。そこで、ある一つのモジュールをインポートすることで一通りの機能を使えるようにしたい時がある。エクスポート宣言は、これを実現する。

このため、 Labda はエクスポート宣言の機能を持つ。

### Labda のインポート宣言とエクスポート宣言は再定義によるものである。

インポート宣言とエクスポート宣言は、型理論において、どのようなものに対応するのだろうか？ たとえば `foo.bar` をインポートするということは、 `bar := foo.bar` というように定義するということだと解釈することができる。インポート宣言とエクスポート宣言の違いは外部へ定義を公開するかの違いである。

このため、 Labda のインポート宣言とエクスポート宣言は再定義によるものである。

### Labda は多様多種な同値性の型と部分型の関係の型を持つ。

Cyril Cohen さんと Enzo Crance さんと Assia Mahboubi さんが 2023 年 10 月 21 日に投稿した論文（[リンク](https://arxiv.org/abs/2310.14022)）によると、 Homotopy Type Theory における等価性の型は、複数の段階において弱めることができるという。この論文の等価性の型とは、大まかに概略すると、型 `A` と `B` の関係において、 `A -> B` の性質と `B -> A` の性質について、それぞれにおいて 6 種類の性質を考え、その合計として 36 種類の関係を定義している。それらは様々な度合いの同値性の型あるいは部分型の関係の型となる。

Labda は、 Homotopy Type Theory における同値性の型だけではなく、変換コストがほぼない同値性の型と簡約で移り合うだけの同値性の型も持つ。これに対する性質も導入しよう。これらの多様多種な同値性と部分型の関係において、それを通した変換を正確に行うためには、それらに対応する type role も必要であるので、これを追加する。このようにすると、様々な関係を柔軟に扱うことができて面白そうである。

このため、 Labda は多様多種な同値性の型と部分型の関係の型を持つ。
...
これらによると、「ファイルを読み込むところ、パーサーのところ……」というように順番に作っていくと、ファイル読み込みやパーサーなどの仕様が揺らぎやすいため、それらに対するテストを変更する頻度が高くなりすぎる。その代わりに、入力の形式と出力の形式だけが定まっている非常に単純な機能だけを持つコンパイラを作り、その後に機能を追加していき、入力と出力だけを対象にしてテストを行うことで、よりよく開発を進めることができる。

このため、全体の枠組みを作ってから細部を広げていく。

### プログラムのファイルを読み込んで計算結果のファイルを出力するプログラムから始める。

最も単純なコンパイラは、なんであるかと考えると、プログラムのファイルを読み込んで計算結果のファイルを出力するプログラムである。これはコンパイラというよりインタプリンタであるが、コンパイラの要素も含む。

このため、プログラムのファイルを読み込んで計算結果のファイルを出力するプログラムから始める。
