...
二つ目に、「名前」を持つ。プログラミング言語は、通常の場合、バッカスナウア記法を用いて定義することができる。しかしながら、自然言語は、生成文法に従う入れ子の構造を基本としているというわけではなく、むしろ小さな部品を平坦に連結して並べていることがほとんどであるように、私は思う。そして、人間が読みやすいプログラムのコードというのも、このような自然言語と同様に、できるだけ入れ子の構造を避けて小さな部品を平坦に連結して並べるという特徴を持っているように、私は思う。このように考えて、小さな部品を平坦に連結して並べることが人間が親しみやすい言語の本質であるのではないかと、私は思う。だが、小さな部品を平坦に連結して並べるだけでは多様な処理を表現することはできない。つまり、まだ足りない要素がある。それは「名前」なのではないかと私は思う。「名前」を使うことにより、多様多種な概念について、それらの網のような繋がりを表せるようになる。

三つ目に、型を追加しやすい。

四つ目に、型と値を同じ表記で表せるため、新しく「型」のための表記を導入する必要がない。依存型も表現しやすく、高度な数学も表現できるようになる。また、ラムダ計算ではない計算モデルに型を追加しようとしても、その「型」のための表記はラムダ計算に似通ったものになってしまい、ラムダ計算を完全に排除しようとすると無限後退になってしまうと、私は思う。

五つ目に、型と値を同じ表記で表せるため、「型」を追加しやすく、「依存型」とも親和する。これは、「それぞれのコードをどのように実行しても、それが他のコードへと影響することがなく、その結果は常に同じである」という性質による。

六つ目に、プログラム自体を値として扱え、どのようなプログラムの部分も関数として抜き出せるため、非常に表現力が高い。これにより、プログラムからプログラムを作るプログラムというような高階関数まで表現することができ、

七つ目に、 Coq や Agda や Lean などもラムダ計算を基本としている。
