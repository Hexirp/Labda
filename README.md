...
Cyril Cohen さんと Enzo Crance さんと Assia Mahboubi さんが 2023 年 10 月 21 日に投稿した論文（[リンク](https://arxiv.org/abs/2310.14022)）によると、 Homotopy Type Theory における等価性の型は、複数の段階において弱めることができるという。この論文は 36 種類の関係を定義しており、それらは様々な度合いの同値性の型と部分型の関係の型である。さらに、これらに対して変換コストがない同値性の型と変換コストがない部分型の関係の型と簡約で移り合うだけの同値性の型を導入して、それらに対応する type role も追加すると、面白いのではないだろうか？
